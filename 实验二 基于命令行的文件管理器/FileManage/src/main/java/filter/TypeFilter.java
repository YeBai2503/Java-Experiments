...
public class TypeFilter implements Filter{
    @Override
    public void sift(CurrentDirectory currentDirectory, String type, long big,long small) {
        ArrayList<AbstractFile> siftFiles = new ArrayList<>();
        // 遍历当前目录中的所有文件
        for (AbstractFile file : currentDirectory.getCurrentFiles()) {
            if(type.equals("文件夹")&&file.getType().equals("文件夹"))//文件夹时
            {
                siftFiles.add(file);
                continue;
            }
            ...
            if (lastDotIndex > 0 && lastDotIndex < fileName.length() - 1) {
                // 提取并返回扩展名
                thisType=fileName.substring(lastDotIndex + 1); // 返回 '.' 之后的部分
            }
