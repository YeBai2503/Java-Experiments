...
public class DecryptFileCommand implements Command{
    ...
    public void execute(CommandDetail details) {
        ...
        if(details.getBack()==1)//后台
        {
            ...
            details.getTaskPool().submitTask("解密 "+details.getSourcePath(), () -> {
                    ...
                    try {
                        ...
                        try (FileChannel channel = FileChannel.open(Paths.get(sourcePath))) {
                            try (FileLock fileLock = channel.lock(0, Long.MAX_VALUE, true)) {
                                ...
                                DecryptFile.decryptFile(details.getPassword(), details.getSourcePath(), details.getDestinationPath(), 1);
            ...
            try {
                DecryptFile.decryptFile(details.getPassword(),details.getSourcePath(),details.getDestinationPath(),0);
            } catch (Exception e) {
                e.printStackTrace();
            }
